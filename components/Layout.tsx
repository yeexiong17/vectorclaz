...
const Layout: React.FC<LayoutProps> = ({ children, role, currentView, setView, onLogout }) => {
  ...
  const adminNav: NavItem[] = [
    ...
    { id: 'ADMIN_ATTENDANCE', label: 'Attendance Details', icon: <Calendar size={20} /> },
        ...
        <div className="flex-1 py-6 px-4 space-y-1">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors text-sm font-medium ${currentView === item.id
